    ...
    $http.get(`http://localhost:3000/quizs/${$routeParams.id}`).then(
        function(res){
            ...
            //Lưu trữ đáp án
                ...
                $scope.toggleChecked = function(model, name, value, type) {
                    if (!$scope.clickCount[value]) {
                        model = true; // Nếu là lần đầu tiên click, đặt giá trị là true
                        $scope.clickCount[value] = 1; // Đánh dấu đã click lần đầu
                    } else {
                        if ($scope.clickCount[value] % 2 === 0) {
                            model = true; // Nếu là lần click chẵn (lần click thứ 4, 6, 8,...), đặt giá trị là true
                        } else {
                            model = false; // Nếu là lần click lẻ (lần click thứ 3, 5, 7,...), đặt giá trị là false
                        }
